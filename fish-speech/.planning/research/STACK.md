# Technology Stack: Streaming Chunked TTS Audio

**Project:** Fish Speech S2-Pro Streaming Audio
**Researched:** 2026-04-12
**Overall confidence:** HIGH (verified against codebase + multiple production implementations)

## Executive Summary

The existing Fish Speech codebase already contains most of the libraries needed for streaming chunked audio. The key additions are: (1) a Hann-window crossfade implementation using numpy (zero new dependencies), (2) minor WAV header modifications for unknown-length streaming, and (3) an overlap context strategy for the DAC decoder. No new heavyweight libraries are required.

The DAC codec in this codebase uses `causal=True` convolutions and a 44.1kHz sample rate with a hop_length of 512 samples (~11.6ms per token). This causal architecture is favorable for chunked decoding because causal convolutions don't look ahead -- but the decoder still needs neighboring context tokens to produce clean boundaries.

## Existing Stack (Already in Codebase)

These are already installed and used in the Fish Speech inference pipeline. Do not replace them.
...
Everything below is implemented using numpy + stdlib. No pip installs required.
...
def hann_crossfade(prev_tail: np.ndarray, curr_head: np.ndarray) -> np.ndarray:
    """Crossfade two audio segments using a Hann window."""
    n = len(prev_tail)  # Must equal len(curr_head)
    t = np.linspace(0, 1, n, dtype=np.float32)
    fade_out = 0.5 * (1.0 + np.cos(np.pi * t))  # 1 -> 0
    fade_in  = 0.5 * (1.0 - np.cos(np.pi * t))  # 0 -> 1
    return prev_tail * fade_out + curr_head * fade_in
```

**Parameters for 44.1kHz (Fish Speech DAC sample rate):**
...
**The problem:** Even though Fish Speech's DAC uses `causal=True`, the convolutional decoder has a receptive field determined by `decoder_rates=[8, 8, 4, 2]`. The `from_indices()` method calls `quantizer.decode(indices)` then `decoder(z)`. Without left context, the first few samples of each chunk may have subtle artifacts.

**The approach (based on DAC's own chunked processing):**

```
Chunk N tokens:     [........CCCCCCCC]
                     ^^^^^^^^         = left context (not emitted, just for decoder warmup)
                             ^^^^^^^^ = actual chunk audio (emitted after crossfade)
```
...
**Critical detail from the codebase:** `self.hop_length = np.prod(encoder_rates)` = 512 samples per token. `self.frame_length = self.hop_length * 4` = 2048 samples. The decoder_rates [8, 8, 4, 2] mirror the encoder_rates [2, 4, 8, 8] in reverse, giving a total upsampling factor of 512x.
...
**Current implementation** (`fish_speech/inference_engine/utils.py`):
The existing `wav_chunk_header()` uses Python's `wave` module which writes a valid WAV header with zero data length. This actually works for streaming because most players/decoders handle the zero-length case.

**Improvement:** Set the RIFF chunk size and data chunk size to `0xFFFFFFFF` (max uint32) instead of 0. This is the standard trick for streaming WAV:
...
**WAV format parameters for this project:**
...
**Keep chunked HTTP. Do NOT switch to WebSocket.** Rationale:
...
WebSocket would only make sense if text arrives incrementally (LLM token streaming) or you need mid-utterance cancellation. For a TTS API where the full text is submitted upfront, chunked HTTP is the correct choice.

**When to reconsider:** If a future milestone adds LLM-to-TTS streaming (text tokens arrive as they're generated), WebSocket becomes the right answer. But that's out of scope here.
...
This isn't a "library" but it's a critical stack decision that affects everything.

**Text chunk size:** The existing `chunk_length` parameter controls `max_bytes` per text batch. Current default is 512 bytes. For streaming, this needs to be tuned:
...
**Recommendation:** Start at 200 bytes for streaming mode. The goal is TTFA <500ms, which means the first chunk must be short enough to generate quickly at ~80 tok/s.

**Audio chunk size (DAC tokens per decode):** This is how many semantic tokens are accumulated before calling `from_indices()`:
...
**Recommendation:** Decode the full text chunk at once (current behavior). The text chunking already controls granularity. Sub-chunk token streaming within a text chunk adds complexity without proportional TTFA benefit, since the first text chunk is already small.

## Architecture Implications

The crossfade happens **between** segments yielded by the inference engine, not within the DAC decoder. The modification points are:

1. **`inference_engine/__init__.py`** -- Add crossfade between consecutive segments
2. **`inference_engine/utils.py`** -- Modify `wav_chunk_header()` for streaming length
3. **`tools/server/inference.py`** -- Already handles segment-by-segment streaming
4. **`tools/server/views.py`** -- No changes needed (StreamResponse already works)

The crossfade state (previous chunk's tail samples) lives in the inference generator's local scope -- no new classes or persistent state needed.
